...
def TurtleStrategy(stock_symbol, start_date, end_date, plot = False):
    print(f'Working on {stock_symbol}')
    ...
    stock_df["position"] = stock_df["buy_position"] - stock_df["sell_position"]
    ...
    stock_df_filtered = stock_df[stock_df.position != 0]
    stock_df_filtered["consider"] = stock_df_filtered['position'].diff()
    stock_df_filtered = stock_df_filtered[(stock_df_filtered.consider.notnull()) & (stock_df_filtered.consider != 0)]
    
    stock_df_filtered["value"] = stock_df_filtered['Open'].diff()
    stock_df_filtered = stock_df_filtered[stock_df_filtered.position < 0] #consider value when there is a sell

    print(stock_df_filtered.head(20))

    sum = stock_df_filtered["value"].sum()

    print(f'Earning in {stock_symbol} = {sum}')
